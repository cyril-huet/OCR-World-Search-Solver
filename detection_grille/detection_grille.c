...
int main(int argc, char** argv)
{
	...
	detectAndDrawGrid(IMG_Load("grid_only.png"));


	if(!(imageSurface->w == 841) && (!(imageSurface->w == 770)))
	{
	detectGridArea(IMG_Load("grid_only.png"), &gridRect);

	// Creates a surface for the grid only
	SDL_Surface* gridSurface = SDL_CreateRGBSurface(0, gridRect.w, gridRect.h, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0);
	SDL_BlitSurface(IMG_Load("grid_only.png"), &gridRect, gridSurface, NULL);
