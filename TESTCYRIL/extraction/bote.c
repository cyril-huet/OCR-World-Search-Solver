...
void group_and_save_letters(Image *src, BoundingBoxList *list, const char *input_filename, const char *prefix_override) {
    // Déterminer le préfixe selon le nouvel argument ou le nom du fichier d'entrée
    const char *prefix = (prefix_override != NULL && strlen(prefix_override) > 0) ? prefix_override : 
                         (strcmp(input_filename, "grid_only.pgm") == 0 ? "letter_grid_" : "letter_list_");
    ...
    for (int l = 0; l <= line_number; l++) {
        ...
        for (int i = start; i < end; i++) {
            ...
            char filename[200];
            snprintf(filename, sizeof(filename), "%s/letter_line%d_col%d.png", output_dir, prefix, l, i - start);
...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 3) {
        fprintf(stderr, "Utilisation: %s <image_entree.pgm> [prefix]\n", argv[0]);
        return 1;
    }

    const char *input_filename = argv[1];
    const char *prefix_override = (argc == 3) ? argv[2] : NULL;
    ...
    Image src = load_pgm_image(input_filename);
    binarize_image(&src);

    BoundingBoxList list = detect_letters(&src, 1, 200);
    group_and_save_letters(&src, &list, input_filename, prefix_override);
