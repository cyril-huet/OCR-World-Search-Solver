...
void set_pixels_black(const char *input_file, const char *output_file, Pixel *pixels, int num_pixels) {
    FILE *fp_in, *fp_out;
    char format[3];
    ...
    // Mettre les pixels spécifiés en noir
    for (int i = 0; i < num_pixels; i++) {
        int x = pixels[i].x;
        int y = pixels[i].y;

        if (x >= 0 && x < width && y >= 0 && y < height) {
            image_data[y * width + x] = 0; // Noir
        } else {
...
int main() {
    ...
    int num_pixels = sizeof(pixels_to_black) / sizeof(Pixel);

    // Mettre les pixels spécifiés en noir
    set_pixels_black(input_file, output_file, pixels_to_black, num_pixels);
