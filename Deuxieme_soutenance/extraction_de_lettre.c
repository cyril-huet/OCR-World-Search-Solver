    ...
    for (int y = 0; y < src->height; y++) {
        for (int x = 0; x < src->width; x++) {
            if (src->data[y * src->width + x] == 0 && !visited[y * src->width + x]) {
                ...
                // Modification: permettre aux lettres étroites et longues (comme "i") d'être acceptées
                if ((width >= min_size && height >= min_size && width <= max_size && height <= max_size) ||
                    (width <= max_size && height >= 2 * width && height <= max_size)) {
...
void group_and_save_letters(Image *src, BoundingBoxList *list) {
    ...
    for (int l = 0; l <= line_number; l++) {
        ...
        for (int i = start; i < end; i++) {
            ...
            char filename[100];
            snprintf(filename, sizeof(filename), "letter_line%d_col%d.png", l, i - start);
...
int main(int argc, char *argv[]) {
    ...
    BoundingBoxList list = detect_letters(&src, 3, 200);
    group_and_save_letters(&src, &list);
