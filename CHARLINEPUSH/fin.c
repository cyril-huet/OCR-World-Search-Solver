...
void sauvegarder_rectangle_image(const char *chemin_image, int x1, int y1, int x2, int y2, int index) {
    ...
    // Définir le rectangle source (partie de l'image à sauvegarder)
    SDL_Rect rect_source = {x1, y1, x2 - x1, y2 - y1};

    // Définir le rectangle de destination (dimensions finales)
    SDL_Rect rect_dest = {0, 0, x2 - x1, y2 - y1};

    // Créer une texture cible pour le rendu
    SDL_Texture *texture_cible = SDL_CreateTexture(rendu, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_TARGET, x2 - x1, y2 - y1);
    if (!texture_cible) {
    ...
    if (sauvegarder_renderer(rendu, x2 - x1, y2 - y1, chemin_sauvegarde) != 0) {
